...
function montar(dados){
  console.log("Dados recebidos para montar:", dados);
  ...
  if(!dados || !dados.length){
    console.warn("⚠️ Nenhum dado disponível para montar o painel.");
    return;
  }
  ...
  dados.forEach(d=>{
    ...
    const dias = Number(d.dias_criticos||0); // Frequência no status (RT)
    const IPT = (peso*0.4)+(pont*0.4)+(dias*0.2);
    somaIPT += IPT;
...
async function carregarAvaliacoes(){
  try{
    ...
    console.log("Total de documentos carregados:", dados.length);
    montar(dados);
  }catch(err){
